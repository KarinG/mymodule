...
class CustomFullcalendarViewPreprocess extends FullcalendarViewPreprocess {
      ...
      foreach ($view->result as $row) {
        ...
        if (!empty($start_dates) && is_array($start_dates)) {
          foreach ($start_dates as $i => $start_date) {
            ...
            // }
            elseif (isset($row->users_field_data_civicrm_uf_match_uid)) {
              $uid = $row->users_field_data_civicrm_uf_match_uid;
              $user = \Drupal\user\Entity\User::load($uid);
              $field_colour = $user->get('field_colour')->getValue();
              $term_id = $field_colour['0']['target_id'];
              $entry['backgroundColor'] = \Drupal\taxonomy\Entity\Term::load($term_id)->get('name')->value;
            }
            // Recurring event.
